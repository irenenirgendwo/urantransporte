...
* Run <code>rake db:setup</code> or <code>rake db:migrate</code> and <code>rake db:seed</code> to create the database.
...
* terminal command to run all tests in root folder: rake test
* rake test:units runs the unit tests, each test for a model to test self defined model actions
* rake test:functionals runs the functional tests which are testing controller actions
* rake test:integration runs the integration tests which define use cases to browse the application. See more in issue #70.
...
Still TODO. Things you may want to cover:
